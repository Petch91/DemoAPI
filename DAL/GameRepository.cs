...
namespace DAL
{
   public class GameRepository : BaseRepository<int, Game>, IGameRepository
   {
      ...
      {
         string sql = "DELETE FROM GameGenre WHERE GameId = @id";
         SqlParameter[] parameters =
         {
         ...
         SqlParameter[] parameters2 =
{
         ...
         return ExecuteNonQuery(sql, CommandType.Text, parameters2) != 0;
      }
      ...
      public IEnumerable<Game> GetFavGames(Guid userId)
      {
         ...
         IEnumerable<Game> lg = ExecuteReader<Game>(sql, CommandType.Text, parameters, reader => Mapper(reader));
         if (lg is null)
         {
            lg = new List<Game>();
         }
